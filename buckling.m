...
elseif strcmp(Shape, 'beam')
    ...
    Bo = input('Input Outer Base of Beam (big side): ');
    Ho = input('Input Outer Width of Beam (small side): ');

    Bi = input('Input Inner Base of Beam (big side): ');
    Hi = input('Input Inner Width of Beam (small side): ');
    
    A = (Bo*Ho)-(Bi*Hi);    % Calculating Cross Sectional Area
    disp('');
    disp('Area of Beam: ');
    disp(A);

    I = ((Bo*Ho^3)/12) - ((Bi*Hi^3)/12);   % Calculating Moment of Inertia for Beam
